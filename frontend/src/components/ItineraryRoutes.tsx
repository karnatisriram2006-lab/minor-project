...
import { Car, Bus, Footprints, Clock, IndianRupee, Navigation, ChevronRight, ChevronDown } from 'lucide-react'
...
const MODES = [
  { id: 'car', icon: <Car className="w-3.5 h-3.5" />, label: 'Car', rate: 12, speedMult: 1.8, bufferMin: 10 },
  { id: 'bus', icon: <Bus className="w-3.5 h-3.5" />, label: 'Bus', rate: 4, speedMult: 2.5, bufferMin: 15 },
  { id: 'walk', icon: <Footprints className="w-3.5 h-3.5" />, label: 'Walk', rate: 0, speedMult: 8, bufferMin: 2 }
];
...
export function ItineraryRoutes({ 
  ...
  // modePreferences maps index -> mode id
  ...
  useEffect(() => {
    ...
    const compute = async () => {
      ...
      const flattenedStops: ({ stop: Stop; dayLabel: string; isStart: boolean })[] = []
      ...
      for (let i = 0; i < flattenedStops.length - 1; i++) {
        ...
        let legData: LegData = { 
          ...
          fromLat: from.lat, fromLng: from.lon, 
          toLat: to.lat, toLng: to.lon,
          isDayStart: flattenedStops[i].isStart,
          dayLabel: flattenedStops[i].dayLabel
        }

        if (Number.isFinite(from?.lat) && Number.isFinite(to?.lat)) {
          const route = await fetchRoute({ lat: from.lat, lon: from.lon }, { lat: to.lat, lon: to.lon })
          if (route && route.distanceKm > 0) {
            ...
            prefs[legIndex] = legData.suggestedMode === 'train' || legData.suggestedMode === 'flight' ? 'car' : legData.suggestedMode;
          } else {
            prefs[legIndex] = 'car'
          }
        } else {
  ...
  const calculateLegDetails = (leg: LegData, idx: number) => {
    ...
    const timeMin = Math.round((leg.baseDurationMin || 0) * (modeConfig as any).speedMult + (modeConfig as any).bufferMin)

    return { modeConfig, distance, cost, timeMin }
  }

  const handleModeChange = (idx: number, modeId: string) => {
    setModePreferences(prev => ({ ...prev, [idx]: modeId }))
  }
    ...
    <div className="flex flex-col bg-white border-t border-[#EBEBEB]">
      ...
      >
        <div className="flex items-center gap-3">
           ...
           <div>
             <h3 className="text-[15px] font-bold text-[#484848] tracking-tight">Transit & Route Details</h3>
             <p className="text-[11px] text-[#767676] font-medium">Distance, commute modes, and estimated costs</p>
           </div>
        </div>
        <ChevronRight className={`w-5 h-5 text-[#BBBBBB] transition-transform ${isExpanded ? 'rotate-90' : ''}`} />
      </div>
      ...
      {isExpanded && (
        <>
          ...
          {loading ? (
            ...
            <div className="text-sm text-[#767676]">No route legs found.</div>
          ) : (
            <div className="space-y-0 relative">
              {legs.map((leg, idx) => {
                const { modeConfig, distance, cost, timeMin } = calculateLegDetails(leg, idx)
                  ...
                  <div key={idx} className="relative group">
                    ...
                    <div className="flex items-center gap-4">
                       ...
                       <div className="flex flex-col">
                          <span className="font-bold text-[15px] text-[#484848] tracking-tight leading-none mb-1">{leg.from}</span>
                          <span className="text-[10px] font-bold text-[#767676] uppercase tracking-wider">Milestone {idx + 1}</span>
                       </div>
                    </div>
                    ...
                    <div className="ml-[7.5px] border-l-2 border-dashed border-[#DDDDDD] pl-8 py-5 relative group-hover:border-[#FF5A5F]/40 transition-colors">
                       ...
                       <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-[#767676] font-semibold mb-4 bg-white/50 rounded-xl p-3 border border-[#EBEBEB] w-fit shadow-sm">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0 text-[#484848]">
                            <Clock className="w-3.5 h-3.5 text-[#00A699]" />
                            <span>{timeMin >= 60 ? `${Math.floor(timeMin/60)}h ${timeMin%60}m` : `${timeMin} mins`}</span>
                          </div>
                          <div className="w-1 h-1 rounded-full bg-[#DDDDDD]" />
                          <div className="flex items-center gap-1.5 shrink-0 text-[#484848]">
                       ...
                       <button 
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            if (onFocusLeg && leg.fromLat && leg.fromLng && leg.toLat && leg.toLng) {
                              onFocusLeg({ 
                    ...
                    {isLast && (
                       <div className="flex items-center gap-4 mt-1">
                         ...
                         <div className="flex flex-col">
                            <span className="font-bold text-[15px] text-[#484848] tracking-tight leading-none mb-1">{leg.to}</span>
                            <span className="text-[10px] font-bold text-[#767676] uppercase tracking-wider">Trip Finish</span>
                         </div>
                       </div>
                    )}
                  </div>
          ...
          {!loading && legs.length > 0 && (
            <div className="px-6 py-6 bg-[#F7F7F7] border-t border-[#EBEBEB]">
              <h4 className="text-xs font-bold text-[#767676] uppercase tracking-widest mb-4">Trip Route Summary</h4>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-white p-4 rounded-2xl border border-[#EBEBEB] shadow-sm flex flex-col items-center justify-center text-center hover:border-[#FF5A5F]/30 transition-colors">
                  <span className="text-xl font-bold text-[#484848] tracking-tight">{totalDistance.toFixed(0)}</span>
                  <span className="text-[9px] uppercase font-bold tracking-[0.2em] text-[#767676] mt-1">Total km</span>
                </div>
                <div className="bg-white p-4 rounded-2xl border border-[#EBEBEB] shadow-sm flex flex-col items-center justify-center text-center hover:border-[#00A699]/30 transition-colors">
                  <span className="text-xl font-bold text-[#484848] tracking-tight">
                    {totalTime >= 60 ? `${Math.floor(totalTime/60)}h ${totalTime%60}m` : `${Math.floor(totalTime)}m`}
                  </span>
                  <span className="text-[9px] uppercase font-bold tracking-[0.2em] text-[#767676] mt-1">Total Time</span>
                </div>
                <div className="bg-white p-4 rounded-2xl border border-[#EBEBEB] shadow-sm flex flex-col items-center justify-center text-center hover:border-[#FF5A5F]/30 transition-colors">
                  <span className="text-xl font-bold text-[#FF5A5F] tracking-tight">₹{totalCost}</span>
                  <span className="text-[9px] uppercase font-bold tracking-[0.2em] text-[#FF5A5F]/70 mt-1">Est. Cost</span>
                </div>
              </div>
            </div>
          )}
