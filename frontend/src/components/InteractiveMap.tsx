...
import { Navigation, MapPin, Search, Shield, Info, ArrowRight, Plus, Minus, Sun, Moon } from 'lucide-react'
import MarkerClusterGroup from 'react-leaflet-cluster'
import type { MultiModeRoute } from '@/services/routeService'

// Custom Components
import { cn } from '@/lib/utils'
import { createNumberedMarker, createUserLocationMarker } from './map/CustomMarker'
...
function MapController({ 
  ...
  useEffect(() => {
    if (focusLeg || routingLeg) {
      ...
      const bounds = L.latLngBounds([[from[0], from[1]], [to[0], to[1]]])
      if (bounds.isValid()) {
      ...
      if (validPoints.length > 0) {
        if (validPoints.length === 1) {
          map.flyTo([validPoints[0].lat, validPoints[0].lng], 12, { animate: true, duration: 1.5 })
        } else {
  ...
  return (
    <div style={{
      ...
      <button
        ...
        onMouseEnter={e => { e.currentTarget.style.background='#f8f9fa'; e.currentTarget.style.transform='scale(1.05)' }}
...
function MyLocationButton({ onLocation }: { onLocation: (pos: [number, number]) => void }) {
  const map = useMap()
      ...
      const fetchDirectRoute = async () => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data.routes?.[0]) {
              const pathCoords: [number, number][] = data.routes[0].geometry.coordinates.map((c: number[]) => [c[1], c[0]])
              setRouteCoordinates(pathCoords)
              return
            }
          }
          setRouteCoordinates([[from[0], from[1]], [to[0], to[1]]])
        } catch (err) {
    ...
    <div style="font-family: 'DM Sans', sans-serif;">
      ...
      <div style="padding: 12px 16px; background: white;">
        <div style="display: flex; align-items: center; gap: 8px;
          ...
          <span style="font-size: 13px;">${stop.time || 'Any time'}</span>
          ${stop.duration ? `
            <span style="color:#ccc">·</span>
            <span style="font-size: 13px;">${stop.duration}</span>
          ` : ''}
        </div>
        ${stop.description ? `
    ...
    return (
      <div className="h-full w-full bg-[#f5f3ef] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#767676]">Loading Map Engine</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="h-full w-full relative bg-[#f5f3ef]">
      ...
      >
        ...
        <MapController points={points} focusLeg={focusLeg} routingLeg={routingLeg} />
        <MapSearchBar />
        ...
        >
          {points.filter(p => !!p).map((stop, index) => (
            <Marker
              key={`${stop.id}-${index}`}
        ...
        {userLocation && (
          ...
          >
            <Popup>
              <div style={{ fontFamily: "'DM Sans',sans-serif", padding: '2px 4px' }}>
                <div style={{ fontWeight: 700, fontSize: 13, color: '#111827', marginBottom: 2 }}>📍 You are here</div>
      ...
      <div className={cn(
        ...
        <div className="flex flex-col gap-2">
           <button 
             onClick={() => document.querySelector('.leaflet-container')?.dispatchEvent(new CustomEvent('map-zoom-in'))}
           ...
           >+</button>
           <button 
             onClick={() => document.querySelector('.leaflet-container')?.dispatchEvent(new CustomEvent('map-zoom-out'))}
             className="h-10 w-10 bg-white rounded-full shadow-[0_4px_15px_rgba(0,0,0,0.12)] border border-[#f0ede8] flex items-center justify-center text-[#484848] font-bold text-lg hover:bg-[#E8391A]/5 hover:text-[#E8391A] transition-all active:scale-95"
           >−</button>
        </div>
      </div>
